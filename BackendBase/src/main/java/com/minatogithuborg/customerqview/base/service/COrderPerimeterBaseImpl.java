...
import com.minatogithuborg.customerqview.base.model.constants.COrderConstantBase;
...
@Component
public abstract class COrderPerimeterBaseImpl<T extends COrderBase> implements IPerimeterManager<T>, COrderConstantBase {
	
	private Logger LOGGER = LoggerFactory.getLogger(COrderPerimeterBaseImpl.class);
	...
	protected void setReadFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			String[] readFields = new String[] {SID, CREATED_BY, CREATED_DATE, MODIFIED_BY, MODIFIED_DATE, ORDER_NO, CUSTOMER_NO, PRODUCT_ID, QUANTITY, PRICE};
	...
	protected void setWriteFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			return;
					...
					if (BooleanUtils.isTrue(userBase.isAppAdmin())) {
			String[] readFields = new String[] {ORDER_NO, CUSTOMER_NO, PRODUCT_ID, QUANTITY, PRICE};
			allowedAccessFields.addAll(Arrays.asList(readFields));
		}
