...
import com.minatogithuborg.customerqview.base.model.constants.CProductConstantBase;
...
@Component
public abstract class CProductPerimeterBaseImpl<T extends CProductBase> implements IPerimeterManager<T>, CProductConstantBase {
	
	private Logger LOGGER = LoggerFactory.getLogger(CProductPerimeterBaseImpl.class);
	...
	protected void setReadFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			String[] readFields = new String[] {SID, CREATED_BY, CREATED_DATE, MODIFIED_BY, MODIFIED_DATE, PRODUCT_ID, PRODUCT_NAME};
	...
	protected void setWriteFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			return;
					...
					if (BooleanUtils.isTrue(userBase.isAppAdmin())) {
			String[] readFields = new String[] {PRODUCT_ID, PRODUCT_NAME};
			allowedAccessFields.addAll(Arrays.asList(readFields));
		}
