...
import com.minatogithuborg.customerqview.base.model.constants.CustomerConstantBase;
...
@Component
public abstract class CustomerPerimeterBaseImpl<T extends CustomerBase> implements IPerimeterManager<T>, CustomerConstantBase {
	
	private Logger LOGGER = LoggerFactory.getLogger(CustomerPerimeterBaseImpl.class);
	...
	protected void setReadFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			String[] readFields = new String[] {SID, CREATED_BY, CREATED_DATE, MODIFIED_BY, MODIFIED_DATE, CUSTOMER_NO, CUSTOMER_NAME, CUSTOMER_ADDRESS, CUSTOMER_CONTACT};
	...
	protected void setWriteFields(ApplicationUserBase userBase, AllowedFields allowedFields) {
		...
		if(userBase == null) {
			...
			return;
					...
					if (BooleanUtils.isTrue(userBase.isAppAdmin())) {
			String[] readFields = new String[] {CUSTOMER_NO, CUSTOMER_NAME, CUSTOMER_ADDRESS, CUSTOMER_CONTACT};
			allowedAccessFields.addAll(Arrays.asList(readFields));
		}
