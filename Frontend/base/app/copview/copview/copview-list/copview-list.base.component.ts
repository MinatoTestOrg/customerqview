...
export class CopviewListBaseComponent{
...
localStorageStateKey = "copview-list";
...
queryViewList:boolean = true; // dynamic Variable has to be updated here
...
@Output() onBeforeValidationEmitter: EventEmitter<any> = new EventEmitter();
  ...
  "currentNode" : "TABLE",
  "detailPageTableId" : "6ebab0ab-4c1a-49e4-a501-0fd2917c9616",
  "columnReorder" : false,
  "type" : "grid",
  "showDetailPageAs" : "navigate_to_new_page",
  "rowGroup" : "yes",
  "outline" : false,
  "detailPageType" : "query",
  "children" : [ {
    "fieldName" : "orderno",
    "data" : "orderNo",
    "formatDisplay" : false,
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "bf1b7121-54e2-425a-8da0-fb2c195945e4",
    "name" : "COPView Detail",
    "url" : "/copview/copviewdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'COPVIEW_LIST';
...
focus(){
  ...
  hasNumbers.forEach((f: any) => {
    ...
    let action: Exclude<keyof CopviewListBaseComponent, ' '> = methodName;
  ...
  return date.getFullYear() + '-' + this.leftPad((date.getMonth() + 1), 2) + '-' + this.leftPad(date.getDate(), 2);
   ...
   let property: Exclude<keyof CopviewListBaseComponent, ' '> = value;
       this.showGrid = ((componentConfig?.length >= 0 && this.existingFormId) || (this.fromDetailPage && this[property]) || this.queryViewList || !this.fromDetailPage);
    if (!componentConfig) {
      return;
    }
...
if (this.holdFilters.length > 0) {
  this.holdFilters.forEach((filter: string) => {
   ...
   if (!(control instanceof FormGroup)) {
      ...
      if(this.fromDetailPage) {
        this.onBeforeValidationEmitter.emit({
          ...
          parentCallbackFunction: ((parentObj: any) => {
            if (parentObj == 'true') {
                this.copviewSharedService.setData({});
                  this.appUtilBaseService.navigateWithQueryParams({ 'isCreate': 'yes' }, this.tableConfig.detailPage?.url)
            }
          })
        });
      } else {
    ...
    else if (!this.queryViewFiltersApplied && this.showonFilter) {
  ...
  this.copviewSharedService.setData(data);
  if(this.fromDetailPage) {
    this.onBeforeValidationEmitter.emit({
      data: this.tableConfig.showDetailPageAs,
      parentCallbackFunction: ((parentObj: any) => {
  ...
  if (formControl.controls[parent] instanceof FormGroup){
	...
	onDelete() {
    const threshold = 10; // Threshold for triggering info message

    if (this.selectedRows.length > threshold) {
      // Display an info message if the number of selected values exceeds the threshold
      this.translateService.get('DELETION_IS_LIMITED_TO_OPENBRACE_OPENBRACE_THRESHOLD_CLOSEBRACE_CLOSEBRACE_RECORDS_DOT_PLEASE_CHOOSE_ACCORDINGLY', { threshold: threshold })
      .subscribe((message: string) => {
        this.showInfoMessage(message);
      });
    } else {
      // Proceed with deletion
      let requestedParams: any = this.selectedRows;
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.copviewService.deletewithBodyParams(requestedParams).subscribe((res: any) => {
  ...
  this.clearFilterValues();
    ...
    let params = this.assignTableParams();
    this.toShowRecords(params);
    if (this.gridComponent && 'refreshGrid' in this.gridComponent) {
        this.gridComponent.refreshGrid(params, fromDel,this.queryViewFiltersApplied);
      }
    this.selectedValues = [];
...
leftPad(num:number, length:number) {
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.copviewService.getProtoTypingData().subscribe((data: any) => {
  ...
  if (event.which === 13 || event.keyCode === 13) {
   ...
   this.onRefresh();
    ...
    this.showonFilter = this.queryViewList || mapConfigFields.length > 0;
  ...
  this.filtersApplied = false;
    ...
    if (this.mapConfig[this.componentId]?.length > 0 && missingMapConfigFields.length > 0) {
      this.hasMappedParameters = false;
    }
    else if(missingMapConfigFields.length  == 0 || this.mapConfig[this.componentId]?.length <= 0){
 ...
 assignTableParams() {
  ...
  disablechildAction(pid?:any) {
    ...
    return {
      ...
      ajaxUrl: CopviewApiConstants.getResults,
      select: true,
