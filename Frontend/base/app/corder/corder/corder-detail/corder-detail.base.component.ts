...
import { CorderBase} from '../corder.base.model';
...
import { CorderApiConstants } from '@baseapp/corder/corder/corder.api-constants';
import { CustomerApiConstants } from '@baseapp/customer/customer/customer.api-constants';
import { CproductApiConstants } from '@baseapp/cproduct/cproduct/cproduct.api-constants';
...
export class CorderDetailBaseComponent{
...
filteredItemsdetailFormcustomerNo:any = [];
isAutoSuggestCallFireddetailFormcustomerNo: boolean = false;
	isChildPage:boolean = false;
	showGrid:boolean = true;
	
filteredItemsdetailFormproductId:any = [];
isAutoSuggestCallFireddetailFormproductId: boolean = false;
  ...
  "children" : [ {
    ...
    "data" : "Customer No",
    "lookupUrl" : "/rest/customers/lookup/customerlookup1",
    "autoSuggestServiceName" : "lookupCustomerLookup1",
    "type" : "formField",
    "mandatory" : "no",
    "displayFields" : [ {
      "name" : "customerNo",
      "sid" : "ae3e75c0-2709-43a8-9898-a5c9ff27bb88",
      "f_type" : "text",
      "tableId" : "f8bbba37-c294-4312-a1c5-c7ba38fb627b"
    } ],
    "filterOutputMapping" : [ ],
    "mandatoryFilters" : [ ],
    "fieldId" : "customerNo",
    ...
    "isPrimaryKey" : false,
    "label" : "CUSTOMER_NO",
    "parentField" : "customerNo",
    "optionalFilters" : [ "customerNo" ],
    "outputFields" : [ ],
    "referenceField" : [ ],
    "field" : "customerNo",
    "transient" : false,
    ...
    "fieldName" : "productId",
    "data" : "Product ID",
    "lookupUrl" : "/rest/cproducts/lookup/cproductlookup1",
    "autoSuggestServiceName" : "lookupCProductLookup1",
    "type" : "formField",
    "mandatory" : "no",
    "displayFields" : [ {
      "name" : "productId",
      "sid" : "b72bb2fa-e9bf-4bce-ac50-60ae3cfe9bdc",
      "f_type" : "text",
      "tableId" : "e504c4ce-6b2a-40f0-b761-f151e8bf6702"
    } ],
    "filterOutputMapping" : [ ],
    "mandatoryFilters" : [ ],
    "fieldId" : "productId",
  ...
  "queryViewMapping" : { }
	...
	mapConfig : any = { }
	pageViewTitle: string = 'CORDER_DETAIL';
	
	public corderService = inject(CorderService);
...
dynamicDialogRef:any;
	...
	orderNo: new UntypedFormControl('',[Validators.required]),
	...
	autoSuggestSearchdetailFormcustomerNo(event?: any, col?: any,url?:any,fromInit?:any) {
col = col || this.formFieldConfig['customerNo'];
    if(environment.prototype){
    this.filteredItemsdetailFormcustomerNo = [];
    for (let i = 1; i <= 4; i++) {
      let obj = {
          id: i,
          ['displayField']: "Sample customerNo" + i
      };
      this.filteredItemsdetailFormcustomerNo.push(obj);
  }
  }
    else{
if(!this.isAutoSuggestCallFireddetailFormcustomerNo){
      this.isAutoSuggestCallFireddetailFormcustomerNo = true;
    let apiObj = Object.assign({}, CustomerApiConstants.lookupCustomerLookup1);
     const urlObj = {
        url: (url || apiObj.url),
  searchText: (event && this.detailFormControls.controls['customerNo'].value) ? event.query == this.detailFormControls.controls['customerNo'].value['displayField'] ? '':event.query: (event ? event.query : ''),     
  formConfig :this.formFieldConfig,
       colConfig: col,
        value: this.detailFormControls.getRawValue(),
        pageNo:this.autoSuggestPageNo
      }
    apiObj.url = this.appUtilBaseService.generateDynamicQueryParams(urlObj,true);
   const sub =  this.baseService.get(apiObj).subscribe((res: any) => {
      this.isAutoSuggestCallFireddetailFormcustomerNo = false;
  res.map((d: any) => {
            let displayFieldconCat: any = [];
            col.displayFields?.map((o: any) => {
               const formattedData = this.appUtilBaseService.formatRawDatatoRedableFormat({}, d[o.name], o.f_type);
              displayFieldconCat.push(formattedData);
            })
            const value = (displayFieldconCat.length > 1) ? displayFieldconCat.join("_") : displayFieldconCat.join();
            d.displayField = value;
          })
          let updateRecords = [];
          if(event && event.query) {
                updateRecords =  [...res];
          } else {
                updateRecords =  [...this.filteredItemsdetailFormcustomerNo, ...res];
          }
     
         if(this.formFieldConfig['customerNo']?.uniqueKeys && this.formFieldConfig['customerNo']?.uniqueKeys?.length > 0){
            const generateIdentifier = (record: any) => this.formFieldConfig['customerNo']?.uniqueKeys?.map((field: string) => record[field]).join('-');
            const identifiers = updateRecords.map(generateIdentifier);
           this.filteredItemsdetailFormcustomerNo = updateRecords.filter((record, index) => {
                const currentIdentifier = identifiers[index];
                return !identifiers.slice(index + 1).includes(currentIdentifier);
            });
          }
       else{
        const lookupFields = this.formFieldConfig['customerNo']?.filterInputMapping?.map((mapping:any) => mapping.lookupField);
     // Function to generate a unique identifier for each record
     const generateIdentifier = (record: { [x: string]: any; }) => lookupFields.map((field: string ) => record[field]).join('-');
     const identifiers = updateRecords.map(record => generateIdentifier(record));
     // Filter out duplicate records based on the unique identifiers
     this.filteredItemsdetailFormcustomerNo = updateRecords?.filter((record, index) => {
       const currentIdentifier = identifiers[index];
       // Check if the current identifier is unique
       return !identifiers.slice(index + 1).includes(currentIdentifier);
     });
    }
          this.filteredItemsdetailFormcustomerNo = this.formatFilteredData(this.filteredItemsdetailFormcustomerNo, 'customerNo');
...
this.subscriptions.push(sub);}
 ...
 getRoleShortName(currentUserData:any){
  ...
  formatCaptionItems(config: any, data: any) {
	...
	attachInfiniteScrollForAutoCompletedetailFormcustomerNo(fieldName:string) {
    ...
    const autoSuggestScrollSubscription = windowYOffsetObservable.subscribe((scrollPos: number) => {
      if ((tracker.offsetHeight + scrollPos >= tracker.scrollHeight)) {
        this.isAutoSuggestCallFireddetailFormcustomerNo = false;
          if(this.filteredItemsdetailFormcustomerNo.length  >= this.autoSuggestPageNo * AppConstants.defaultPageSize){
            this.autoSuggestPageNo = this.autoSuggestPageNo + 1;
          }
         const methodName: any = `autoSuggestSearchdetailFormcustomerNo`
        let action: Exclude<keyof CorderDetailBaseComponent, ' '> = methodName;
        this[action]();
      }
    });
	...
	editFormula(config:any,fieldname:any){
         ...
         if(fieldConfig.formulaMethod){
           let methodName: Exclude<keyof CorderDetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
workflowInformation.step = workflowDataForNewRec.step || '';
	...
	autoSuggestSearchdetailFormproductId(event?: any, col?: any,url?:any,fromInit?:any) {
col = col || this.formFieldConfig['productId'];
    if(environment.prototype){
    this.filteredItemsdetailFormproductId = [];
    for (let i = 1; i <= 4; i++) {
      let obj = {
          id: i,
          ['displayField']: "Sample productId" + i
      };
      this.filteredItemsdetailFormproductId.push(obj);
  }
  }
    else{
if(!this.isAutoSuggestCallFireddetailFormproductId){
      this.isAutoSuggestCallFireddetailFormproductId = true;
    let apiObj = Object.assign({}, CproductApiConstants.lookupCProductLookup1);
     const urlObj = {
        url: (url || apiObj.url),
  searchText: (event && this.detailFormControls.controls['productId'].value) ? event.query == this.detailFormControls.controls['productId'].value['displayField'] ? '':event.query: (event ? event.query : ''),     
  formConfig :this.formFieldConfig,
       colConfig: col,
        value: this.detailFormControls.getRawValue(),
        pageNo:this.autoSuggestPageNo
    ...
    apiObj.url = this.appUtilBaseService.generateDynamicQueryParams(urlObj,true);
   const sub =  this.baseService.get(apiObj).subscribe((res: any) => {
      this.isAutoSuggestCallFireddetailFormproductId = false;
  res.map((d: any) => {
            let displayFieldconCat: any = [];
            col.displayFields?.map((o: any) => {
               const formattedData = this.appUtilBaseService.formatRawDatatoRedableFormat({}, d[o.name], o.f_type);
          ...
          if(event && event.query) {
                ...
                updateRecords =  [...this.filteredItemsdetailFormproductId, ...res];
          }
     
         if(this.formFieldConfig['productId']?.uniqueKeys && this.formFieldConfig['productId']?.uniqueKeys?.length > 0){
            const generateIdentifier = (record: any) => this.formFieldConfig['productId']?.uniqueKeys?.map((field: string) => record[field]).join('-');
            const identifiers = updateRecords.map(generateIdentifier);
           this.filteredItemsdetailFormproductId = updateRecords.filter((record, index) => {
                const currentIdentifier = identifiers[index];
                return !identifiers.slice(index + 1).includes(currentIdentifier);
            });
          }
       else{
        const lookupFields = this.formFieldConfig['productId']?.filterInputMapping?.map((mapping:any) => mapping.lookupField);
     // Function to generate a unique identifier for each record
     const generateIdentifier = (record: { [x: string]: any; }) => lookupFields.map((field: string ) => record[field]).join('-');
     const identifiers = updateRecords.map(record => generateIdentifier(record));
     // Filter out duplicate records based on the unique identifiers
     this.filteredItemsdetailFormproductId = updateRecords?.filter((record, index) => {
       ...
       return !identifiers.slice(index + 1).includes(currentIdentifier);
          ...
          this.filteredItemsdetailFormproductId = this.formatFilteredData(this.filteredItemsdetailFormproductId, 'productId');
...
addValidations(mandatoryFields:[]){
  ...
  if (this.appUtilBaseService.isEqualIgnoreCase(this.backupData, this.detailFormControls.getRawValue(), this.appUtilBaseService.getIgnorableFields(this.detailFormControls.getRawValue(),this.backupData,this.detailPageIgnoreFields), true)) {
	...
	onSelectdetailForm(event: any, field: string, config: any, index?: number) {
 const model = config.parentField ? event[config.parentField] : event['displayField']
  ...
  unSelectdetailForm(event: any, field: string, multiple: boolean, config:any) {
 this.selectedItems[field] = [];
  }
	getDateTimeFields() {
...
onChangelog() {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.corderService[method](requestedObj, actionName).subscribe((res: CorderBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.corderService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.corderService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
saveFiles(splittedData: any) {
  ...
  if (environment.prototype && this.id) {
    ...
    this.corderService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const params = {
      sid: this.id
        ...
        const dataSubscription = this.corderService.getById(params).subscribe((res: CorderBase[]) => {
          this.data = res || {};
	...
	attachInfiniteScrollForAutoCompletedetailFormproductId(fieldName:string) {
    const tracker = (<HTMLInputElement>document.getElementsByClassName('p-autocomplete-panel')[0]);
    if(!tracker) return;
    let windowYOffsetObservable = fromEvent(tracker, 'scroll').pipe(map(() => {
      return Math.round(tracker.scrollTop);
    ...
    const autoSuggestScrollSubscription = windowYOffsetObservable.subscribe((scrollPos: number) => {
      if ((tracker.offsetHeight + scrollPos >= tracker.scrollHeight)) {
        this.isAutoSuggestCallFireddetailFormproductId = false;
          if(this.filteredItemsdetailFormproductId.length  >= this.autoSuggestPageNo * AppConstants.defaultPageSize){
            this.autoSuggestPageNo = this.autoSuggestPageNo + 1;
          }
         const methodName: any = `autoSuggestSearchdetailFormproductId`
        let action: Exclude<keyof CorderDetailBaseComponent, ' '> = methodName;
        this[action]();
      }
    });
	...
	formatAutoComplete(item: any, config: any, formControlName: string) {
