...
export class CorderListBaseComponent{
...
localStorageStateKey = "corder-list";
...
tooltipText:string =''
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "3e12b325-0817-4744-9567-5b883882b271",
    "name" : "COrder Detail",
    "url" : "/corder/corderdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'CORDER_LIST';
...
public location = inject(Location);
    ...
    let action: Exclude<keyof CorderListBaseComponent, ' '> = methodName;
...
focus(){
  this.isSearchFocused = !this.isSearchFocused;
    ...
    return {
      ...
      ajaxUrl: CorderApiConstants.getDatatableData,
      select: true,
...
getParentId() {
  ...
  let property: Exclude<keyof CorderListBaseComponent, ' '> = value;
  if (this.isChildPage) {
    if (this[property]) {
  ...
  disablechildAction(pid?:any) {
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.corderService.delete(requestedParams).subscribe((res: any) => {
 ...
 assignTableParams() {
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.corderService.getProtoTypingData().subscribe((data: any) => {
