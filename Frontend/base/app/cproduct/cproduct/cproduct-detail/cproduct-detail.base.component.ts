...
export class CproductDetailBaseComponent{
...
outputMappingFieldsConfig:any ={};
 ...
 queryParams:any ={};
  ...
  "queryViewMapping" : { }
	...
	mapConfig : any = { }
	pageViewTitle: string = 'CPRODUCT_DETAIL';
...
onChangelog() {
    ...
    let action: Exclude<keyof CproductDetailBaseComponent, ' '> = methodName;
  ...
  showConfirmationPopup(config: any, btn: any) {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.cproductService[method](requestedObj, actionName).subscribe((res: CproductBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.cproductService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.cproductService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
saveFiles(splittedData: any) {
  ...
  if (environment.prototype && this.id) {
    ...
    this.cproductService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const params = {
      sid: this.id
        ...
        const dataSubscription = this.cproductService.getById(params).subscribe((res: CproductBase[]) => {
          this.data = res || {};
   ...
   patchFormControl(key: string, value: any) {
	...
	editFormula(config:any,fieldname:any){
         ...
         if(fieldConfig.formulaMethod){
           let methodName: Exclude<keyof CproductDetailBaseComponent, ' '> = fieldConfig.formulaMethod
