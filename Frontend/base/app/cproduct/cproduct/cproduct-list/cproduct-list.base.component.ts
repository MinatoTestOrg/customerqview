...
export class CproductListBaseComponent{
...
localStorageStateKey = "cproduct-list";
...
tooltipText:string =''
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "e160b787-9765-4cb7-8b2c-c7597e92820f",
    "name" : "CProduct Detail",
    "url" : "/cproduct/cproductdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'CPRODUCT_LIST';
	
	public cproductService = inject(CproductService);
...
focus(){
  this.isSearchFocused = !this.isSearchFocused;
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.cproductService.delete(requestedParams).subscribe((res: any) => {
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.cproductService.getProtoTypingData().subscribe((data: any) => {
  ...
  disablechildAction(pid?:any) {
      ...
      let property: Exclude<keyof CproductListBaseComponent, ' '> = value;
    ...
    let action: Exclude<keyof CproductListBaseComponent, '' > = methodName;
...
this.location.back();
    ...
    return {
      ...
      ajaxUrl: CproductApiConstants.getDatatableData,
      select: true,
