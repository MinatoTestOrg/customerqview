...
export class CustomerListBaseComponent{
...
localStorageStateKey = "customer-list";
...
@Output() onBeforeValidationEmitter: EventEmitter<any> = new EventEmitter();
  ...
  "sortOrder" : "asc",
  "detailPage" : {
    "sid" : "7281119d-69b4-4432-b07b-fddc9046a83b",
    "name" : "Customer Detail",
    "url" : "/customer/customerdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'CUSTOMER_LIST';
...
public location = inject(Location);
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.customerService.delete(requestedParams).subscribe((res: any) => {
...
focus(){
  this.isSearchFocused = !this.isSearchFocused;
    ...
    return {
      ...
      ajaxUrl: CustomerApiConstants.getDatatableData,
      select: true,
  ...
  getSelectedvalues(selectedRows: any, id: string) {
    ...
    let action: Exclude<keyof CustomerListBaseComponent, ' '> = methodName;
  ...
  return date.getFullYear() + '-' + this.leftPad((date.getMonth() + 1), 2) + '-' + this.leftPad(date.getDate(), 2);
    ...
    let property: Exclude<keyof CustomerListBaseComponent, '' > = value;
...
if (this.holdFilters.length > 0) {
  ...
  if (formControl.controls[parent] instanceof FormGroup){
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.customerService.getProtoTypingData().subscribe((data: any) => {
