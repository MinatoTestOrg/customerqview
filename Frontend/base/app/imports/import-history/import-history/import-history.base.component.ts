...
customStatusField(data:any,row:any ){
  ...
  else if (data == 'IMPORT_FAILED_NO_TABLE') {
    data = "IMPORT_FAILED_COLON_TABLE_DOES_NOT_EXIST";
    customStatus = this.translateService.instant("IMPORT_FAILED_COLON_TABLE_DOES_NOT_EXIST")
  }
  ...
  getSearchData(searchFields: any, config: any) {
    ...
    for (const key in searchFields) {
      if (searchFields.hasOwnProperty(key) && searchFields[key]?.toString().length) {
        ...
        else if (key == 'importStatus' && searchFields[key].includes('IMPORT_FAILED_COLON_TABLE_DOES_NOT_EXIST')) {
          const index = searchFields[key].indexOf("IMPORT_FAILED_COLON_TABLE_DOES_NOT_EXIST");
          if (index > -1) {
            searchFields[key].splice(index, 1);
          }
          searchData[key] = [...searchFields[key], 'IMPORT_FAILED_NO_TABLE'];
        }
        else {
