...
export class GridBaseComponent {
  ...
  getData(drawNextPage?: boolean, fromSorting?: boolean, fromRefresh?: boolean, fromPreviousPage?: boolean, fromDelete?: boolean): void {
    const tbl = this.dtInstance?.table();
    let triggerAPI: boolean = true
    if (environment.prototype || (this.showonFilter && !this.filtersApplied)) {
      triggerAPI = false;
    }
    ...
    if (triggerAPI) {
      ...
      if (this.gridConfig.isChildPage || this.gridConfig.fromDetailPage) {
        $("#" + this.Id + '_wrapper ' + '.dataTables_scrollBody').css('max-height', '50px');
      }
